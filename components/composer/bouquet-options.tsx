        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              {option}
              {suffix ? (
                <span className="ml-1 opacity-70">{suffix}</span>
              ) : null}
            </button>
          );
        })}
...
export function BouquetOptions() {
  ...
  const setHandwrittenCard = useBouquetStore(
    (state) => state.setHandwrittenCard,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <OptionGroup<Wrapping>
        ...
        hint={(option) =>
          WRAPPING_PRICE[option] === 0
            ? "inclus"
            : `+ ${formatEuro(WRAPPING_PRICE[option])}`
        }
      />
      <OptionGroup<DeliveryMode>
        ...
        hint={(option) =>
          DELIVERY_PRICE[option] === 0
            ? "gratuit"
            : `${formatEuro(DELIVERY_PRICE[option])}`
        }
      />
      <p className="-mt-3 text-xs text-muted-foreground">
        Livraison locale offerte dès {formatEuro(FREE_LOCAL_DELIVERY_FROM)} de
        commande.
      </p>
      ...
      <div className="space-y-3 border-t border-border pt-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <Switch
            ...
            onCheckedChange={setHandwrittenCard}
          />
        </div>
        <div className="flex items-center justify-between gap-4">
          ...
          <Switch
            id="ruban"
            checked={customRibbon}
            onCheckedChange={setCustomRibbon}
          />
        </div>
      </div>
    </div>
